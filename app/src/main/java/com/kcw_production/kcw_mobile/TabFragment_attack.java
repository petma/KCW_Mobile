...
public class TabFragment_attack extends Fragment {
    ...
    //Test Data
    String [] mapList = {
            "1. 鎮守府海域","1-1 鎮守府正面海域", "1-2 南西諸島沖", "1-3 製油所地帯沿岸", "1-4 南西諸島防衛線", "1-5 [Extra] 鎮守府近海", "1-6 [Extra Operation] 鎮守府近海航路",
    ...
    //// FIXME: 2015/8/25 NullPointerException occur here, I believe. —— Wafer(Too tired to fix)
    private ArrayList<MapCategory> getData() {
        ArrayList<MapCategory> tempMapCategories = new ArrayList<MapCategory>();
        ...
        for(int i = 0,j = 0;i<mapList.length;i++) {
            ...
            if(mapList[i].contains(".")) {
                ...
                tempMapCategories.get(j).addItem(mapList[i]);
            }
